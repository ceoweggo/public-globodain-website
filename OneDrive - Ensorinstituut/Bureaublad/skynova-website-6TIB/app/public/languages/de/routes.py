...
@register_breadcrumb(public_language, '.de_index', 'Globodain')
def index():
...
def add_user_to_newsletter():
    ...
    if gd.db.users_newsletter.find_one({'email': email}) is not None:
        flash(f'Die E-Mail {email} ist bereits in unserem Newsletter eingetragen.', 'danger')
    else:
        from datetime import datetime
        ...
        flash(f'Die E-Mail {email} wurde in unseren Newsletter aufgenommen. Prüfen Sie Ihren Posteingang, wir haben Ihnen eine Bestätigungs-E-Mail geschickt.', 'success')
    return redirect('/')
...
class navbar:
    ...
    @register_breadcrumb(public_language, '.de_education', 'Ausbildung', 3)
    def education():
...
class gtc_explore_navbar:
    ...
    @register_breadcrumb(public_language, '.de_applications', 'Anwendungen')
    def applications():
...
class sections:
    ...
    @register_breadcrumb(public_language, '.de_applications.de_finances', 'Finanzen')
    ...
    @register_breadcrumb(public_language, '.de_applications.de_accounting', 'Buchhaltung')
    ...
    @register_breadcrumb(public_language, '.de_applications.de_business', 'Business Intelligence')
    ...
    @register_breadcrumb(public_language, '.de_applications.de_city30', 'City 3.0')
    ...
    @register_breadcrumb(public_language, '.de_services', 'Dienstleistungen')
    def services():
    ...
    @register_breadcrumb(public_language, '.de_education.de_students', 'Für Studenten')
    def education_students():
        ...
        if request.method == 'POST':
            ...
            if not gd.db.requests.find_one({"type": "contact", "email": items['email']}):
                ...
                emails.send_no_reply_email(items['email'], "Ein Student hat eine Petition eingereicht", 
                                        "<p>Liebe Schule:</p>Wir kontaktieren Sie von <strong>Globodain</strong>, einem Technologieunternehmen aus Spanien, auf Anfrage eines Ihrer Schüler, um eine mögliche <strong>Zusammenarbeit in den Bildungsprogrammen</strong> zu diskutieren, die wir für Schüler und Lehrer anbieten.<br>Unsere Bildungsprogramme, in Zusammenarbeit mit europäischen Regierungsinstitutionen, sind vollständig durch die regionalen Entwicklungspläne und die wirtschaftliche Zusammenarbeit der Europäischen Union finanziert.<br>Wenn dies für Sie von Interesse wäre, würde ich mich freuen, Ihnen weitere Informationen über unsere Programme zukommen zu lassen.<br><p style='color: #004AAD;'>** Sie können uns schreiben, wenn Sie interessiert sind, an ana.perez@globodain.com oder ruben.ayuso@globodain.com<p><br>Mit freundlichen Grüßen, <br>Rubén Ayuso<br>Programmdirektor von <a href='https://globodain.com'>Globodain Education</a><br><b><small>Dies ist eine automatische E-Mail, die vom internen System der Plattform gesendet wurde. Bitte nicht auf diese E-Mail antworten</small>")
                
                flash('Vielen Dank, wir werden uns mit Ihrer Schule in Verbindung setzen, um unsere Bildungsprogramme anzubieten.', 'success')
            else:
                flash('Sie haben bereits einen Antrag gestellt. Dranbleiben', 'danger')
    ...
    @register_breadcrumb(public_language, '.de_education.de_schools', 'Für Schulen')
    def education_schools():
        ...
        if request.method == 'POST':
            ...
            if not gd.db.requests.find_one({"type": "contact", "email": items['email']}):
                ...
                flash('Vielen Dank für Ihre Kontaktaufnahme, ein Mitglied des Teams wird Sie für ein erstes Gespräch kontaktieren.', 'success')
            else:
                flash('Sie haben bereits einen Antrag gestellt. Dranbleiben', 'danger')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs', 'Bildungsprogramme')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility', 'Internationale Mobilität')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_erasmus', 'Erasmus+')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_services', 'Servicios')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_services.de_host', 'Prácticas')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_services.de_accommodation', 'Alojamiento')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_services.de_trips', 'Viajes')
    ...
    @register_breadcrumb(public_language, '.de_education.de_programs.de_mobility.de_services.de_experiences', 'Experiencias')
    ...
    @register_breadcrumb(public_language, '.de_education.de_areas', 'Áreas de enseñanza')
    def education_areas():
        return render_template("sections/education/es-partnership.html", 
                               lang_data=find_data('index'),
                               title="Educational Partnership")
    ...
    @register_breadcrumb(public_language, '.de_education.de_areas.de_programming', 'Programación')
    ...
    @register_breadcrumb(public_language, '.de_education.de_areas.de_marketing', 'Marketing')
    ...
    @register_breadcrumb(public_language, '.de_education.de_areas.de_uiux', 'Interfaz de usuarios')
    ...
    @register_breadcrumb(public_language, '.de_education.de_partnership', 'Ich möchte ein Partner sein')
    ...
    @register_breadcrumb(public_language, '.de_education.de_partnership.de_request_prices', 'Preise anfragen')
    def education_partnership_request_prices():
        if request.method == 'POST':
            ...
            if not gd.db.requests.find_one({"type": "prices", "email": items['email']}):
                ...
                flash('Es wurde eine Anfrage an das Team gesendet', 'success')
            else:
                flash('Sie haben bereits früher Preise angefordert. Dranbleiben', 'danger')
...
class pages:
    ...
    @register_breadcrumb(public_language, '.de_request', 'Dienstleistungen anfordern')
        ...
        if request.method == 'POST':
            ...
            if gd.db.contact_blacklist.find_one({"fullname": fullname}) or gd.db.contact_blacklist.find_one({"email": email}):
                ...
                if gd.db.users_contact_request.find_one({"email": email}):
                    flash(f'Sie haben bereits einen Antrag gestellt. Dranbleiben', 'danger')
                else:
                    from datetime import datetime   
                    ...
                    if not next_page or url_parse(next_page).netloc != '':
                        flash(f'Ihre Anfrage wurde registriert und ein Mitglied des Teams wird sich in Kürze mit Ihnen in Verbindung setzen.', 'success')
    ...
    @public_language.route("/sitemap/")
    @register_breadcrumb(public_language, '.de_sitemap', 'Sitemap')
    def sitemap():
